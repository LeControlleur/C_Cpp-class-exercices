...
int main(int argc, char *argv[])
{
    ...
    while (TRUE)
    {
        ...
        if (FD_ISSET(master_socket, &readfds))
        {
            ...
            //  Envoi d'un message au nouveau client
            if (send(new_socket, message, strlen(message), 0) != strlen(message))
            {
                perror("envoi message");
            }

            printf("Le message de connexion a bel et bien été envoyé\n");
        ...
        for (i = 0; i < max_clients; i++)
        {
            ...
            if (FD_ISSET(sd, &readfds))
            {
                ...
                else
                {
                    ...
                    //  Structure nous donnant le type de message reçu
                    struct json_object *type_message;
                    char *type_message_str;

                    parsed_json = json_tokener_parse(buffer);

                    json_object_object_get_ex(parsed_json, "type_message", &type_message);
                    
                    type_message_str = "VERIF_ADMIN";

                    //  sprintf(type_message_str, "%s", type_message);

                    if (strcmp(type_message_str, "VERIF_ADMIN") != 0)
                    {
                        //  Fonction de vérification de l'administration
                        response = malloc(BUFFER_SIZE);
                        response = is_admin(parsed_json, mdp_admin);

                        printf("La réponse reçu du server est : %s", response);
                        //  Action à effectuer en fonction du type de message
                    }
                    else
                    {
                        response = malloc(BUFFER_SIZE);
                        response = "Réponse vide\0";
                    }
